...
ON m.memid = b.memid
WHERE f.facid IN (0,1) AND firstname != 'GUEST'
...
SELECT firstname,surname,name,
 cost FROM (SELECT name, surname, CASE WHEN firstname = 'GUEST'
THEN guestcost * slots
ELSE membercost * slots
END AS cost
FROM Bookings AS b
...
SELECT m2.firstname, m2.surname, m2.recommendedby, m1.firstname, m1.surname
FROM Members AS m1, Members AS m2
WHERE m2.recommendedby = m1.memid
AND m1.firstname != 'Guest' AND m2.firstname != 'Guest'
ORDER BY m2.surname, m2.firstnamey

/* Q12: Find the facilities with their usage by member, but not guests */


SELECT name, firstname, surname, COUNT(b.memid) AS count
FROM Facilities AS f
...
SELECT name, EXTRACT(MONTH FROM starttime) AS month, COUNT(EXTRACT(MONTH FROM starttime)) AS count
FROM Facilities AS f
LEFT JOIN Bookings AS b
ON f.facid = b.facid
GROUP BY month, name
